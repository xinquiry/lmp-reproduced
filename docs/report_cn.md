...
### 3.5 结果分析
*   **Al/Mg 体系**：计算值 (622 mJ/m²) 高于文献值 (~289 mJ/m²)，且在100k到300k步退火过程中未见显著改善。这表明界面结构已达到亚稳态平衡。偏差可能源于：(1) 有限尺寸效应限制了长程失配位错网络的完全松弛；(2) 界面构建方式产生的特定堆垛序列。
*   **Ti/TiB2 体系**：长时退火后的PE/atom (-5.95 eV) 介于纯Ti (-4.87 eV) 和 TiB2 (-5.6~-7.6 eV) 之间，表明界面区域已形成稳定的过渡结构，原子环境介于两相之间。
